...
call sp_ver_compras
call sp_crear_cliente('nombre','21212121','123@gmial','12345');
call sp_crear_cliente('aru3','12322455','adrunh67@gmail.com','123dd');
select*from usuario
...
DELIMITER $$
USE `b54994_lenguajes_proy2`$$
CREATE DEFINER=`laboratorios`@`%` PROCEDURE `sp_crear_cliente`(IN nombre VARCHAR(100), IN id int(9), in correo varchar(50),in contraseña varchar(30))
    NO SQL
begin
    start transaction;	
INSERT INTO usuario(nombre,identificador,correo,contraseña,tipo_usuario) values(nombre,id,correo,contraseña,'1');
...
select*from tpermiso_x_trol

select*from rol

insert into usuario(nombre,identificador,correo,tipo_usuario,contraseña) values('Román2','30550022','roman123@gmail.com','0','12345');
insert into rol(ID_user,Descripcion) values('30550022','PRUEBAROMAN');

call sp_crear_cliente(nombre,identificador,correo,tipo_usuario, contraseña)values('Dabrin38','305550555','dabrinlte@gmail.com','dab123')
...
CREATE PROCEDURE SP_ADD_ESTUDIANTE(id_est_in INTEGER,nombre_in VARCHAR(30),apellido_in VARCHAR(30),edad_in INTEGER,carrera_in VARCHAR(20))
AS BEGIN INSERT INTO estudiante(id_est, nombre, apellido, edad, carrera)VALUES(id_est_in, nombre_in, apellido_in, edad_in, carrera_in);END



SET TERM ^ ;
CREATE PROCEDURE SP_ADD_ESTUDIANTE (id_est_in INTEGER,nombre_in VARCHAR(30),apellido_in VARCHAR(30),edad_in INTEGER,carrera_in VARCHAR(20)) AS BEGIN INSERT INTO ESTUDIANTE(id_est, nombre, apellido, edad, carrera) VALUES  (:id_est_in,:nombre_in ,:apellido_in,:edad_in,:carrera_in);WHEN sqlcode -530 do exception error_accesos_alta;END
SET TERM ; ^
...
COMMENT ON PROCEDURE SP_ADD_ESTUDIANTE IS 'adding Estudiante';
COMMENT ON PARAMETER SP_ADD_ESTUDIANTE.id_est IS '12456';
COMMENT ON PARAMETER SP_ADD_ESTUDIANTE.apellido IS 'Najera';
COMMENT ON PARAMETER SP_ADD_ESTUDIANTE.edad IS '22';
COMMENT ON PARAMETER SP_ADD_ESTUDIANTE.carrera IS 'Informatica';
...
SELECT *FROM ESTUDIANTE where NOT IN 